...
export default function PresentValueCalculatorPage() {
  ...
  const fvPercent = Math.min(100, Math.max(0, (parsedFv / 50000000) * 100));
      ...
      <main className="flex-grow max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-stretch mb-10">
          ...
          <div className="lg:col-span-7 h-full bg-[var(--calc-card-bg)] border border-[var(--calc-border)] rounded-xl p-6 sm:p-7 space-y-6">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center gap-4">
                <div>
                  <label htmlFor="pv-fv" className="text-[15px] font-semibold text-[var(--calc-text-primary)] block">
                    Target future value (FV)
                  </label>
                  <span className="text-[11px] text-[var(--calc-text-muted)]">Future target amount desired</span>
                </div>
              ...
              {fvWords && <div className="text-xs font-medium text-[var(--calc-accent)] text-right">{fvWords}</div>}
              <input
                type="range"
                min="0"
                max="50000000"
            ...
            <div className="space-y-3 pt-5 border-t border-[var(--calc-border)]">
              <div className="flex justify-between items-center gap-4">
                <div>
                  <label htmlFor="pv-rate" className="text-[15px] font-semibold text-[var(--calc-text-primary)] block">
                    Discount / return rate (% p.a.)
                  </label>
                  <span className="text-[11px] text-[var(--calc-text-muted)]">Assumed annual discount or return rate</span>
                </div>
                <div className="flex items-center rounded-lg border border-[var(--calc-border-input)] bg-[var(--calc-input-bg)] px-3 py-1.5 focus-within:border-[var(--calc-accent)] focus-within:ring-1 focus-within:ring-[var(--calc-accent)] transition-all">
                  <input
                    id="pv-rate"
            ...
            <div className="space-y-3 pt-5 border-t border-[var(--calc-border)]">
              <div className="flex justify-between items-center gap-4">
                <div>
                  <label htmlFor="pv-years" className="text-[15px] font-semibold text-[var(--calc-text-primary)] block">
                    Time horizon (years)
                  </label>
                  <span className="text-[11px] text-[var(--calc-text-muted)]">Number of years until target date</span>
                </div>
                <div className="flex flex-col items-end">
                  <div className="flex items-center rounded-lg border border-[var(--calc-border-input)] bg-[var(--calc-input-bg)] px-3 py-1.5 focus-within:border-[var(--calc-accent)] focus-within:ring-1 focus-within:ring-[var(--calc-accent)] transition-all">
                    <input
          ...
          <div className="lg:col-span-5 h-full bg-[var(--calc-result-bg)] border border-[var(--calc-border)] rounded-xl p-6 sm:p-7 flex flex-col justify-between space-y-6 min-h-[360px]">
            <div>
              <span className="text-[11px] font-semibold text-[var(--calc-text-muted)] uppercase tracking-wider block">Required Present Value (PV)</span>
            ...
            <div className="space-y-3.5 pt-4 border-t border-[var(--calc-border)] text-xs font-semibold">
              <div className="flex justify-between text-[var(--calc-text-secondary)]">
                <span className="font-medium">Target Future Value</span>
                <span className="font-bold text-[var(--calc-text-primary)] tabular-nums">₹{formatIndianNumber(Math.round(result.futureValue))}</span>
              </div>
              <div className="flex justify-between text-[var(--calc-text-secondary)]">
                <span className="font-medium">Compounded Growth Earned</span>
                <span className="font-bold text-[var(--calc-accent)] tabular-nums">₹{formatIndianNumber(Math.round(result.discountAmount))}</span>
              </div>
              <div className="flex justify-between pt-2 border-t border-[var(--calc-border)] text-[var(--calc-text-secondary)]">
                ...
                <span className="font-bold text-[var(--calc-text-primary)] tabular-nums">{(parsedFv > 0 ? result.presentValue / parsedFv : 0).toFixed(4)}</span>
              </div>
            </div>

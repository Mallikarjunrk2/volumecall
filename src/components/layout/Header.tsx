...
export function Header() {
  ...
  const linkClass = (href: string) => {
    ...
    return `text-xs transition-colors duration-150 ${
      active
    ...
    <header className="sticky top-0 z-40 w-full border-b border-[var(--border)] bg-[var(--background)]/85 backdrop-blur-md">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-8 shrink-0">
          <Link href="/" className="flex items-center space-x-2 focus:outline-none focus-visible:ring-1.5 focus-visible:ring-teal-600 dark:focus-visible:ring-teal-450 rounded-xs">
      ...
      <div className="sm:hidden w-full px-4 pb-2 bg-[var(--background)] border-b border-[var(--border)]">
        <SearchAutocomplete placeholder="Search stocks..." />
      </div>
    </header>
  );

...
export function LatestBlogsSection({ articles, isMobileBottom = false }: LatestBlogsSectionProps) {
  if (!articles || articles.length === 0) {
    return null;
  }

  // Mobile layout rendered inline after article body
  if (isMobileBottom) {
    return (
      <section className="space-y-3 pt-6 border-t border-[var(--border-subtle)]">
        <div className="flex items-center space-x-2 pb-1.5 border-b border-[var(--border-subtle)]">
        ...
        <div className="divide-y divide-[var(--border-subtle)]">
          {articles.map((art) => (
            <Link
              key={art.id}
              href={`/blog/${art.slug}`}
              className="group flex items-start gap-3 py-2.5 first:pt-1 last:pb-1 transition-colors"
            >
              {/* Thumbnail 16:9 on Left: ~80px x ~54px */}
              <div className="w-[80px] h-[54px] shrink-0 rounded-xs overflow-hidden bg-neutral-100 dark:bg-neutral-900 border border-[var(--border-subtle)]">
                {art.featured_image ? (
  ...
  // Desktop Sidebar (Unboxed, sits right beside article body, compact horizontal cards)
  return (
